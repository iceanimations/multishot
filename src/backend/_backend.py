...
# Embedded file name:
# C:/Users/qurban.ali.ICE-144/Documents/maya/scripts\shot_subm\src\backend\_backend.py
...
def showNameLabel():
    ...
    pc.headsUpDisplay(
            __HUD_LABEL__, section=2, block=0, blockSize='large',
            dfs='large', command=label)
    pc.headsUpDisplay(
            __HUD_USERNAME__, section=3, block=0, blockSize='large',
            dfs='large', command=getUsername)


def showDate():
    global __HUD_DATE__
    pc.headsUpDisplay(
            __HUD_DATE__, section=1, block=0, blockSize='large', dfs='large',
