...
# Embedded file name:
# C:/Users/qurban.ali.ICE-144/Documents/maya/scripts\shot_subm\src\backend\playblast.py
...
def playblast(data):
    pc.playblast(
            st=data['start'], et=data['end'], f=data['path'], fo=True,
...
def showNameLabel():
    ...
    pc.headsUpDisplay(
            __HUD_LABEL__, section=2, block=pc.headsUpDisplay(nfb=2),
            blockSize='large', dfs='large', command=label)
    if pc.headsUpDisplay(__HUD_USERNAME__, q=True, exists=True):
        pc.headsUpDisplay(__HUD_USERNAME__, remove=True)
    pc.headsUpDisplay(
            __HUD_USERNAME__, section=3, block=pc.headsUpDisplay(nfb=3),
            blockSize='large', dfs='large', command=getUsername)
...
def showDate():
    ...
    pc.headsUpDisplay(
            __HUD_DATE__, section=1, block=pc.headsUpDisplay(nfb=1),
            blockSize='large', dfs='large',
...
class PlayblastExport(Action):
    ...
    def perform(self, readconf=True, **kwargs):
        if self.enabled:
            ...
            if not kwargs.get('hdOnly'):
                self.makePlayblast(
                        sound=kwargs.get('sound'), local=kwargs.get('local'))
    ...
    def makePlayblast(self, item=None, sound=None, hd=False, local=False):
        ...
        pc.playblast(
                format='qt', fo=1, st=item.getInFrame(),
                et=item.getOutFrame(), f=tempFilePath, s=str(sound[0]),
                sequenceTime=0, clearCache=1, viewer=0, showOrnaments=1, fp=4,
                percent=100, compression='H.264', quality=100,
                widthHeight=exportutils.getDefaultResolution(), offScreen=1)
        tempFilePath += '.mov'
        if hd:
            depth = 4
